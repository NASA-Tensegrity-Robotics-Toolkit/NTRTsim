...
namespace
{
  ...
  const struct Config
  {
  ...
  } c =
    {
      ...
      0.5,              // restitution (?)
      1.75*sf,         // pretension (kg-m/s^2) -> set to 4 * 613, the previous value of the rest length controller
...
void PrismModel::setup(tgWorld& world)
{
  ...
  */
  ...
  const tgRod::Config motorConfig(c.radius, c.density, c.friction, 
				c.rollFriction, c.restitution);
  const tgRod::Config tankConfig(tankRadius, 6507/pow(sf,3)/2, c.friction, //density calculated so tank - 8.5 kg
  				 c.rollFriction, c.restitution);
  ...
  tgBasicActuator::Config tankLinkConfig(c.stiffness*10, c.damping, c.pretension, c.hist, 
					 c.maxTens, c.targetVelocity);
					 /*
...
void PrismModel::addRods(tgStructure& s, int offset)
{
  ...
  */
  /*
  //Motors at center of Rods
  s.addPair(0+offset+12, 1+offset+12, "rod"); // 0
  s.addPair(3+offset+12, 2+offset+12, "rod"); // 1
  s.addPair(4+offset+12, 5+offset+12, "rod"); // 2
  s.addPair(7+offset+12, 6+offset+12, "rod"); // 3
  s.addPair(8+offset+12, 11+offset+12, "rod"); // 4
  s.addPair(9+offset+12, 10+offset+12, "rod"); // 5
  */
