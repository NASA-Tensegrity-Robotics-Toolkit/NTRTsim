...
namespace
{
  ...
  double sf = 28; //scaling factor. Match with App file and Controller file
  ...
  const struct Config
  {
  ...
  } c =
    {
      ...
      0.5,              // restitution (?)
      1.55*sf/1.25,         // pretension (kg-m/s^2) -> set to 4 * 613, the previous value of the rest length controller
...
void PrismModel::setup(tgWorld& world)
{
  ...
  */
  ...
  const tgRod::Config tankConfig(tankRadius, 4575/pow(sf,3), c.friction, //density calculated so tank - 8.5 kg
  				 c.rollFriction, c.restitution);
  ...
  */
  ...
  tgBasicActuator::Config tankLinkConfig(c.stiffness, c.damping, c.pretension/2, c.hist, 
					 c.maxTens, c.targetVelocity);
					 /*
