...
void PrismModel::setup(tgWorld& world)
{
  ...
  //THRUSTER
  int thrusterNode = globalOffset; //Save the node position before adding the thruster
  ...
  */
  ...
  for (int i = 0; i < actuators.size(); i++) {
    ...
    allCables.push_back(PrismModel::find<tgBasicActuator>(tgString("muscle num", i))[0]);
  }
  ...
  //Get links for inner hinge, construct inner hinge constraint
  ...
  //Get links for outer hinge, construct outer hinge constraint
  ...
  //Below: Important to set pivot location relative to orientation (i.e. flipping structure required neg. sign in from of pivot position)
...
std::vector<tgBasicActuator*>& PrismModel::getAllCables()
{
  return allCables;
}
...
void PrismModel::addRing(tgStructure& s, double radius, int nPts, int& pointOffset){
  //Add nPts nodes in a circular layout
  ...
  pointOffset += nPts; //update global node count
}
...
void PrismModel::addBottomStructure(tgStructure& s,double extRadius,double payLength, double tankToOuterRing, int& pointOffset){
  ...
  pointOffset += 2; //update global node count
}
...
void PrismModel::makeLinks(tgStructure& s, double extRadius, double intRadius, double tankToOuterRing, double tankRadius, int pointOffset1, int& pointOffset2, int baseStartLink, int gimbalStart){
  ...
  pointOffset2 += 4; //update global node count
}
...
void PrismModel::addThruster(tgStructure& s, int nExt, int& globalOffset, int gimbalStart){
  ...
  globalOffset += 3; //update global node count
}
...
void PrismModel::addStrings(tgStructure& s, int baseStartLink, int nPtBeforeRobot){ 
  ...
  // inner cables
  s.addPair( 0+nPtBeforeRobot,  baseStartLink, tgString("string num", 0)); //24
  s.addPair( 4+nPtBeforeRobot,  baseStartLink, tgString("string num", 1)); //25
  s.addPair( 8+nPtBeforeRobot,  baseStartLink, tgString("string num", 2)); //26
  s.addPair( 6+nPtBeforeRobot, baseStartLink+1, tgString("string num", 3)); //27
  s.addPair( 10+nPtBeforeRobot,  baseStartLink+1, tgString("string num", 4)); //28
  s.addPair( 2+nPtBeforeRobot,  baseStartLink+1,tgString("string num", 5)); //29 
}
...
double* PrismModel::returnCOM(tgStructure &s, int firstNode, int numberofNodes){
  ...
  //Returns centroid of rods (COM if we assume mass of each rod is identical)
