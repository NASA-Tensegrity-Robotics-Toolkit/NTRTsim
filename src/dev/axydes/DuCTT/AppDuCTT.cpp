...
void handleOptions(int argc, char **argv)
{
    ...
    desc.add_options()
        ...
        ("controller,c", po::value<bool>(&add_controller), "Attach the controller to the model. Default = no")
        ("duct,d", po::value<bool>(&add_duct), "Add the duct to the simulation. Default = no")
