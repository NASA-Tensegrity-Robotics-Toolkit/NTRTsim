...
/**
 * @file CrossArmModel.cpp
 * @brief Contains the implementation of class CrossArmModel. Purpose is to allow a more controlled yaw movement.
 * $Id$
 */
...
CrossArmModel::CrossArmModel() : tgModel() {}

CrossArmModel::~CrossArmModel() {}

void CrossArmModel::addNodes(tgStructure& s) {
    const double scale = 0.5;
    const double bone_scale = 0.3;
    const size_t nNodes = 19; 
    ...
    //Added 7/26/15
    //The z coordinate can alter the length of the cross elbow horizontally speaking
    nodePositions.push_back(btVector3(0, -g, a/2)); //17
    nodePositions.push_back(btVector3(0, -g, -a/2)); //18
...
void CrossArmModel::addRods(tgStructure& s) {   
    // ulna and radius
    s.addPair(8,  12,  "rod");
    s.addPair(12,  14,  "rod");
    s.addPair(12,  13,  "rod");
...
void CrossArmModel::addMuscles(tgStructure& s) {
    ...
    s.addPair(17, 3, "olecranon muscle"); //NB actually fascial tissue
    s.addPair(18, 4, "olecranon muscle"); //NB actually fascial tissue
	...
	//added 7/26/15
	s.addPair(17, 13, "olecranon muscle"); //NB actually fascial tissue
	s.addPair(18, 14, "olecranon muscle"); //NB actually fascial tissue
}
 
/*
void CrossArmModel::addMarkers(tgStructure &s) {
...
void CrossArmModel::setup(tgWorld& world) {
...
void CrossArmModel::step(double dt)
{
    // Precondition
    if (dt <= 0.0) {
...
void CrossArmModel::onVisit(tgModelVisitor& r)
{
    tgModel::onVisit(r);
}

const std::vector<tgBasicActuator*>& CrossArmModel::getAllMuscles() const
{
    return allMuscles;
}
    
void CrossArmModel::teardown()
{
    notifyTeardown();
    tgModel::teardown();
}
