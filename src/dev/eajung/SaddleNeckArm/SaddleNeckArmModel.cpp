...
*/
/**
* @file SaddleNeckArmModel.cpp
* @brief Contains the implementation of class SaddleNeckArmModel. Everything  is massless for design purposes. Will implement mass soon.
* Measurements are scaled to average adult male
* $Id$
*/
// This module
#include "SaddleNeckArmModel.h"
// This library
...
SaddleNeckArmModel::SaddleNeckArmModel() : tgModel()
{
}
SaddleNeckArmModel::~SaddleNeckArmModel()
{
}
void SaddleNeckArmModel::addNodes(tgStructure& s)
{
const double scale = 0.5;
const double bone_scale = 0.3;
const size_t nNodes = 15 + 4 + 8 + 6; //4 for massless rod 8 for shoulder 6 for humerus neck
...
nodePositions.push_back(btVector3(0, c-x/2, 0)); // 6** altered
...
nodePositions.push_back(btVector3(0, c+3*a/5-x/2, g)); // 17
nodePositions.push_back(btVector3(0, c+3*a/5-x/2, -g)); // 18

//Shoulder nodes (scapula)
nodePositions.push_back(btVector3(-c/2 + g, c-x/2, g)); // 19
nodePositions.push_back(btVector3(-c/2 + g, c-x/2, -g)); // 20
nodePositions.push_back(btVector3(-c/2 -g, c-x/2, 0)); // 21
nodePositions.push_back(btVector3(-c/2 -g-a*3/2, c-x/2, 0)); // 22
nodePositions.push_back(btVector3(-c/2 -g-a*3/2, c-2*g-x/2, 0)); // 23

//Shoulder nodes (clavicle)
nodePositions.push_back(btVector3(-c, c+a-x/2, 0)); // 24
nodePositions.push_back(btVector3(2*g, c+a-x/2, 0)); // 25
nodePositions.push_back(btVector3(0, c+a-x/2, 0)); // 26

//neck nodes (humerus)
nodePositions.push_back(btVector3(-c/2 - g, c + 3*g/4 -x/2, 0)); // 27
nodePositions.push_back(btVector3(-c/2 - g, c - 3*g/4 -x/2, 0)); // 28
nodePositions.push_back(btVector3(-c/2 + g*.60, c-x/2, 0)); // 29
nodePositions.push_back(btVector3(-c/2 + g*1.60, c-x/2, 0)); // 30
nodePositions.push_back(btVector3(c/6, c-x/2, g)); // 31
nodePositions.push_back(btVector3(c/6, c-x/2, -g)); // 32
...
void SaddleNeckArmModel::addRods(tgStructure& s)
{
// ulna and radius
s.addPair(8, 12, "rod");
s.addPair(12, 14, "rod");
...
//humerus neck (8/3/15)
s.addPair(27, 29, "humerus massless");
s.addPair(28, 29, "humerus massless");
s.addPair(29, 30, "humerus massless");
s.addPair(30, 31, "humerus massless");
s.addPair(30, 32, "humerus massless");
...
void SaddleNeckArmModel::addMuscles(tgStructure& s)
{
...
//Saddle muscles
s.addPair(17, 31, "olecranon muscle");
s.addPair(18, 32, "olecranon muscle");
s.addPair(17, 30, "olecranon muscle");
s.addPair(18, 30, "olecranon muscle");

s.addPair(19, 31, "olecranon muscle");
s.addPair(20, 32, "olecranon muscle");

s.addPair(22, 27, "olecranon muscle");
s.addPair(23, 28, "olecranon muscle");

s.addPair(19, 28, "olecranon muscle");
s.addPair(20, 28, "olecranon muscle");
s.addPair(19, 27, "olecranon muscle");
s.addPair(20, 27, "olecranon muscle");
s.addPair(27, 21, "olecranon muscle");
s.addPair(28, 21, "olecranon muscle");



}
/*
void SaddleNeckArmModel::addMarkers(tgStructure &s)
{
...
*/
void SaddleNeckArmModel::setup(tgWorld& world)
{
...
void SaddleNeckArmModel::step(double dt)
{
// Precondition
if (dt <= 0.0) {
...
void SaddleNeckArmModel::onVisit(tgModelVisitor& r)
{
tgModel::onVisit(r);
}
const std::vector<tgBasicActuator*>& SaddleNeckArmModel::getAllMuscles() const
{
return allMuscles;
}
void SaddleNeckArmModel::teardown()
{
notifyTeardown();
tgModel::teardown();
}
