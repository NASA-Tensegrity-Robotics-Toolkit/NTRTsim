...
/**
 * @file BaseSpineCPGControl_commented.cpp
 * @brief A commented version of the controller script to adapt to a 12 Bar tensegrity structure
...
// This module
#include "BaseSpineCPGControl.h" /* Change .h file */
...
#include "tgCPGActuatorControl.h" 		/* Could be removed */
/* Include tgBasicActuator.h, tgString.h, tgtags.h , tgDataObserver.h */
...
/* Add cassert, stdexcept, vector, iostream */
...
/* ... to here is simply CPG sanity checks */
 ...
 */
BaseSpineCPGControl::BaseSpineCPGControl(BaseSpineCPGControl::Config config,	 /* The ocnfig part could probably be eliminated. */
						std::string args,		 /* This function is called in .h file */
  ...
  nodeLearning(false),  					/* Should this be 'true' if machine learning is activated? */
  edgeLearning(false),
...
/* Here, ths initializeActuators functions needs to be called for the YAML files */

void BaseSpineCPGControl::onSetup(BaseSpineModelLearning& subject)	 /* Change name to TensegrityModel */
{
    // Maximum number of sub-steps allowed by CPG			/* Remove */
	m_pCPGSys = new CPGEquations(200);				/* Remove */
    ...
    setupCPGs(subject, nodeParams, edgeParams);				/* Change function to setupController or similar */
    ...
    m_updateTime = 0.0;				/* What is this? */
    bogus = false;
}

void BaseSpineCPGControl::setupCPGs(BaseSpineModelLearning& subject, array_2D nodeActions, array_4D edgeActions) /* Change name */
{											/* This whole function can probably be */
	    										/* from other controller file */
...
void BaseSpineCPGControl::onStep(BaseSpineModelLearning& subject, double dt) 	/* This function should also be able to be copied */
{
    m_updateTime += dt;
...
void BaseSpineCPGControl::onTeardown(BaseSpineModelLearning& subject)  /* This function is where the fun stuff happens */
{									/* Carefully study it. */
    scores.clear();
