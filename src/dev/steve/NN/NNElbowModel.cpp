...
/**
 * @file NNElbowModel.cpp
 * @brief Contains the implementation of class NNElbowModel. See README
 * $Id$
 */

// This module
#include "NNElbowModel.h"
// This library
...
NNElbowModel::NNElbowModel() : tgModel() {}

NNElbowModel::~NNElbowModel() {}

void NNElbowModel::addNodes(tgStructure& s) {
    const double scale = 0.5;
    const double bone_scale = 0.3;
    const size_t nNodes = 15 + 2 + 1; //2 for massless rod, 1 for tracker
    ...
    nodePositions.push_back(btVector3(b+a/2+0.5, -g, 0)); //17
...
void NNElbowModel::addRods(tgStructure& s) {   
    // ulna and radius
    s.addPair(8,  17,  "forearm tracker rod");
    s.addPair(8,  12,  "forearm rod");
    s.addPair(12,  14,  "rod");
    s.addPair(12,  13,  "rod");
...
void NNElbowModel::addMuscles(tgStructure& s) {
    const std::vector<tgStructure*> children = s.getChildren();

    s.addPair(0, 3, "olecranon muscle"); //NB actually connective tissue
    s.addPair(0, 4, "olecranon muscle"); //NB actually connective tissue
    s.addPair(1, 3, "olecranon muscle"); //NB actually connective tissue
    s.addPair(1, 4, "olecranon muscle"); //NB actually connective tissue
    s.addPair(2, 3, "olecranon muscle"); //NB actually connective tissue
    s.addPair(2, 4, "olecranon muscle"); //NB actually connective tissue
    
    s.addPair(0, 13, "olecranon muscle"); //NB actually connective tissue
    s.addPair(1, 13, "olecranon muscle"); //NB actually connective tissue
    s.addPair(11, 13, "olecranon muscle"); //NB actually connective tissue
    s.addPair(0, 14, "olecranon muscle"); //NB actually connective tissue
    s.addPair(1, 14, "olecranon muscle"); //NB actually connective tissue
    s.addPair(11, 14, "olecranon muscle"); //NB actually connective tissue

    s.addPair(0, 12, "olecranon muscle"); //NB actually connective tissue
    s.addPair(11, 12, "olecranon muscle"); //NB actually connective tissue
    s.addPair(0, 5, "brachioradialis muscle");
    s.addPair(2, 5, "olecranon muscle"); //NB actually connective tissue
    s.addPair(3, 13, "right anconeus muscle");
    s.addPair(4, 14, "left anconeus muscle");
}
 
void NNElbowModel::addMarkers(tgStructure &s) {
    //std::vector<tgRod *> rods=find<tgRod>("rod");
    std::vector<tgRod *> forearm = find<tgRod>("forearm");
    
	for(size_t i=0; i<forearm.size(); i++) {
		//const btRigidBody* bt = rods[rodNumbersPerNode[i]]->getPRigidBody();
		const btRigidBody* bt = forearm[i]->getPRigidBody();
...
void NNElbowModel::setup(tgWorld& world) {
    ...
    // Add marker to track end-effector
    //addMarkers(s);

}

void NNElbowModel::step(double dt)
{
    // Precondition
    if (dt <= 0.0) {
...
void NNElbowModel::onVisit(tgModelVisitor& r)
{
    tgModel::onVisit(r);
}

const std::vector<tgBasicActuator*>& NNElbowModel::getAllMuscles() const
{
    return allMuscles;
}
    
void NNElbowModel::teardown()
{
    notifyTeardown();
    tgModel::teardown();
}
