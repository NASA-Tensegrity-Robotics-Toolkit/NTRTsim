...
void LengthController::onSetup(SUPERballV2Model& subject)
{
...
void LengthController::onStep(SUPERballV2Model& subject, double dt)
{
    if (dt <= 0.0) {
    ...
    static bool packing = true;
    // delta as a function of time:
    double delta = -(55.0-88.0)*(globalTime-2.0)/5.0 + 55.0;
    if (!packing) delta = (55.0-88.0)*(globalTime-2.0)/5.0 + 88.0;
    ...
    if (delta > 88.0) { delta = 88.0;
        if (packing) {
            packing = false; // Now, deploy.
            globalTime = 0;
        }
    }
    //alpha = 60; delta=55;
    
    alpha = 60.0/180.0 * M_PI;
    ...
    double T_scaling = 1.0 / T_B * 2000.0; // Cap tension at 2000.
    T_S *= T_scaling;
    T_V *= T_scaling;
    T_D *= T_scaling;
    T_B *= T_scaling;
...
void LengthController::setRestLengthWithCmdTensionLength(int actuatorID, double tension_cmd, double length_cmd, double dt)
{
    ...
    // Rest length control:
    double commandRestLen = length_cmd - tension_cmd/(m_springCable->getCoefK()) - 0.05*10.0;
