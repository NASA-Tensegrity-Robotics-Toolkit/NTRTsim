...
tgCPGStringControl_mod::tgCPGStringControl_mod(const tgPIDController::Config pid_config, const double controlStep) :
tgCPGCableControl(pid_config, controlStep)
{
...
void tgCPGStringControl_mod::onStep(tgSpringCableActuator& subject, double dt)
{
    ...
    tgKinematicActuator& m_sca = *(tgCast::cast<tgSpringCableActuator, tgKinematicActuator>(subject));
    ...
    if (m_controlTime >= m_controlStep)
    {
		...
		if (m_nodeNumber == 0 || m_nodeNumber == 1 || m_nodeNumber == 2 || m_nodeNumber == 4 || m_nodeNumber == 5 || m_nodeNumber == 6 || m_nodeNumber == 7 || m_nodeNumber == 8)//||m_nodeNumber == 9|| m_nodeNumber == 10 )
		{
