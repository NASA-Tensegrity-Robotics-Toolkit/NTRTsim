...
AppMGControl::AppMGControl(int argc, char** argv)
{
    bSetup = false;
    use_graphics = false;
    add_controller = true;
...
bool AppMGControl::setup()
{
    ...
    if (add_controller)
    {
        const int segmentSpan = 3;//3; //Not sure what this will be for mine!
	...
	const double minH = 1.0;
        ...
        JSONMGFeedbackControl::Config control_config(segmentSpan, 
                                                    numMuscles,
                                                    numMuscles,
                                                    numParams, 
                                                    segNumber, 
...
void AppMGControl::handleOptions(int argc, char **argv)
{
...
const tgBoxGround::Config AppMGControl::getBoxConfig()
{
...
tgModel* AppMGControl::getBlocks()
{
...
tgWorld* AppMGControl::createWorld()
{
...
tgSimViewGraphics *AppMGControl::createGraphicsView(tgWorld *world)
{
    return new tgSimViewGraphics(*world, timestep_physics, timestep_graphics);
}

tgSimView *AppMGControl::createView(tgWorld *world)
{
    return new tgSimView(*world, timestep_physics, timestep_graphics);
}

bool AppMGControl::run()
{
    if (!bSetup)
    {
        setup();
...
void AppMGControl::simulate(tgSimulation *simulation)
{
...
{
    std::cout << "AppMGControl" << std::endl;
    AppMGControl app (argc, argv);
