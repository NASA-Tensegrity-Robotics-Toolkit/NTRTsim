...
/**
 * @file JSONQuadCPGGeneralControl.cpp
...
JSONQuadCPGGeneralControl::Config::Config(int ss,
										int tm,
										int om,
										int param,
										int segnum,
 ...
 */
JSONQuadCPGGeneralControl::JSONQuadCPGGeneralControl(JSONQuadCPGGeneralControl::Config config,	
												std::string args,
...
JSONQuadCPGGeneralControl::~JSONQuadCPGGeneralControl() 
{
    scores.clear();
}

void JSONQuadCPGGeneralControl::onSetup(BaseQuadModelLearning& subject)
{
...
void JSONQuadCPGGeneralControl::setupCPGs(BaseQuadModelLearning& subject, array_2D nodeActions, array_4D edgeActions)
{
...
#else
		tgCPGGeneralActuatorControl* pStringControl = new tgCPGGeneralActuatorControl();
    ...
    for (std::size_t i = 0; i < m_allControllers.size(); i++)
    {
        tgCPGGeneralActuatorControl * const pStringInfo = m_allControllers[i];
...
void JSONQuadCPGGeneralControl::onStep(BaseQuadModelLearning& subject, double dt)
{
    m_updateTime += dt;
...
void JSONQuadCPGGeneralControl::onTeardown(BaseQuadModelLearning& subject)
{
    scores.clear();
...
const double JSONQuadCPGGeneralControl::getCPGValue(std::size_t i) const
{
	// Error handling on input done in CPG_Equations
	return (*m_pCPGSys)[i];
}

double JSONQuadCPGGeneralControl::getScore() const
{
	if (scores.size() == 2)
	{
		return scores[0];
...
array_4D JSONQuadCPGGeneralControl::scaleEdgeActions  
...
array_2D JSONQuadCPGGeneralControl::scaleNodeActions  
                            (Json::Value actions)
{
