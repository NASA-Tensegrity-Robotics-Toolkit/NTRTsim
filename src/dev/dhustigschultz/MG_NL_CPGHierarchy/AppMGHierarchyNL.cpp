...
#include "AppMGHierarchyNL.h"
#include "dev/btietz/JSONTests/tgCPGJSONLogger.h"

AppMGHierarchyNL::AppMGHierarchyNL(int argc, char** argv)
{
    bSetup = false;
    use_graphics = false;
    add_controller = true;
...
bool AppMGHierarchyNL::setup()
{
    ...
    MountainGoatNL* myModel = new MountainGoatNL(segments, hips, legs);
	...
	const double maxH = 50.0;
	const double minH = 7.0;

	const double numHipMuscles = 10; 
	const double numLegMuscles = 11; 
	...
	const double ffMax2 = 10.0;
        ...
        JSONHierarchyControlNL::Config control_config(segmentSpan, 
                                                    numMuscles,
                                                    numMuscles,
                                                    numParams, 
                                                    segNumber, 
...
void AppMGHierarchyNL::handleOptions(int argc, char **argv)
{
...
const tgHillyGround::Config AppMGHierarchyNL::getHillyConfig()
{
    ...
    size_t ny = 100;
    double margin = 0.5;
    double triangleSize = 7.0;
    double waveHeight = 2.0;
    double offset = 0.0; //This is in the y-direction
...
const tgBoxGround::Config AppMGHierarchyNL::getBoxConfig()
{
...
tgModel* AppMGHierarchyNL::getBlocks()
{
    // Room to add a config
     btVector3 origin = btVector3(0.0, 0.0, 0.0);
     btScalar friction = 0.5;
     btScalar restitution = 0.0;
     btVector3 minPos = btVector3(-300.0, 0.0, -300.0);
     btVector3 maxPos = btVector3(300.0, 0.0, 300.0);
     size_t nBlocks = 500;
     double blockLength = 20.0;
     double blockWidth = 20.0;
     double blockHeight = 7.0;
...
tgWorld* AppMGHierarchyNL::createWorld()
{
...
tgSimViewGraphics *AppMGHierarchyNL::createGraphicsView(tgWorld *world)
{
    return new tgSimViewGraphics(*world, timestep_physics, timestep_graphics);
}

tgSimView *AppMGHierarchyNL::createView(tgWorld *world)
{
    return new tgSimView(*world, timestep_physics, timestep_graphics);
}

bool AppMGHierarchyNL::run()
{
    if (!bSetup)
    {
        setup();
...
void AppMGHierarchyNL::simulate(tgSimulation *simulation)
{
...
{
    std::cout << "AppMGHierarchyNL" << std::endl;
    AppMGHierarchyNL app (argc, argv);
