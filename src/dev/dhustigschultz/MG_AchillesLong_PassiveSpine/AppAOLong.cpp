...
AppAOLong::AppAOLong(int argc, char** argv)
{
    bSetup = false;
    use_graphics = false;
    add_controller = true;
...
bool AppAOLong::setup()
{
    ...
    MountainGoatAchillesLong* myModel = new MountainGoatAchillesLong(segments, hips, legs);
	...
	const double maxH = 100.0;//30.0;
	const double minH = 7.0;

        JSONAOLongControl::Config control_config(segmentSpan, 
                                                    numMuscles,
                                                    numMuscles,
                                                    numParams, 
                                                    segNumber, 
       ...
       myControl =
        new JSONAOLongControl(control_config, suffix, lowerPath); //JSONAOLongControl * const 
...
void AppAOLong::handleOptions(int argc, char **argv)
{
...
const tgHillyGround::Config AppAOLong::getHillyConfig()
{
...
const tgBoxGround::Config AppAOLong::getBoxConfig()
{
...
tgSimViewGraphics *AppAOLong::createGraphicsView(tgWorld *world)
{
    return new tgSimViewGraphics(*world, timestep_physics, timestep_graphics);
}

tgSimView *AppAOLong::createView(tgWorld *world)
{
...
void AppAOLong::simulate(tgSimulation *simulation)
{
...
{
    std::cout << "AppAOLong" << std::endl;
    AppAOLong app (argc, argv);
