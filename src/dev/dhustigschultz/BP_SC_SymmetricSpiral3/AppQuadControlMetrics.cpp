...
/**
 * @file AppQuadControlMetrics.cpp
...
#include "AppQuadControlMetrics.h"
#include "dev/btietz/JSONTests/tgCPGJSONLogger.h"

AppQuadControlMetrics::AppQuadControlMetrics(int argc, char** argv)
{
    bSetup = false;
    use_graphics = false;
    add_controller = true;
...
bool AppQuadControlMetrics::setup()
{
    ...
    BigPuppySymmetricSpiralMetrics* myModel = new BigPuppySymmetricSpiralMetrics(segments, hips, legs);
	...
	const double minH = 1.0;
        ...
        JSONMetricsFeedbackControl::Config control_config(segmentSpan, 
                                                    numMuscles,
                                                    numMuscles,
                                                    numParams, 
                                                    segNumber, 
...
void AppQuadControlMetrics::handleOptions(int argc, char **argv)
{
...
const tgHillyGround::Config AppQuadControlMetrics::getHillyConfig()
{
...
const tgBoxGround::Config AppQuadControlMetrics::getBoxConfig()
{
...
tgModel* AppQuadControlMetrics::getBlocks()
{
...
tgWorld* AppQuadControlMetrics::createWorld()
{
...
tgSimViewGraphics *AppQuadControlMetrics::createGraphicsView(tgWorld *world)
{
    return new tgSimViewGraphics(*world, timestep_physics, timestep_graphics);
}

tgSimView *AppQuadControlMetrics::createView(tgWorld *world)
{
    return new tgSimView(*world, timestep_physics, timestep_graphics);
}

bool AppQuadControlMetrics::run()
{
    if (!bSetup)
    {
        setup();
...
void AppQuadControlMetrics::simulate(tgSimulation *simulation)
{
...
{
    std::cout << "AppQuadControlMetrics" << std::endl;
    AppQuadControlMetrics app (argc, argv);
