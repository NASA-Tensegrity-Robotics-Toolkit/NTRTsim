...
AppAAS::AppAAS(int argc, char** argv)
{
    bSetup = false;
    use_graphics = false;
    add_controller = true;
...
bool AppAAS::setup()
{
    ...
    MountainGoatAchillesSpirals* myModel = new MountainGoatAchillesSpirals(segments, hips, legs);
	...
	const double ffMax2 = 10.0;
        ...
        JSONAASFeedbackControl::Config control_config(segmentSpan, 
                                                    numMuscles,
                                                    numMuscles,
                                                    numParams, 
                                                    segNumber, 
       ...
       myControl =
        new JSONAASFeedbackControl(control_config, suffix, lowerPath);  
...
void AppAAS::handleOptions(int argc, char **argv)
{
...
const tgHillyGround::Config AppAAS::getHillyConfig()
{
...
const tgBoxGround::Config AppAAS::getBoxConfig()
{
...
tgSimViewGraphics *AppAAS::createGraphicsView(tgWorld *world)
{
    return new tgSimViewGraphics(*world, timestep_physics, timestep_graphics);
}

tgSimView *AppAAS::createView(tgWorld *world)
{
...
void AppAAS::simulate(tgSimulation *simulation)
{
...
{
    std::cout << "AppAAS" << std::endl;
    AppAAS app (argc, argv);
