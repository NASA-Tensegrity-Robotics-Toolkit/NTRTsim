...
#include "AppSpineControl.h"
#include "dev/btietz/JSONTests/tgCPGJSONLogger.h"

AppSpineControl::AppSpineControl(int argc, char** argv)
{
    bSetup = false;
    use_graphics = false;
    add_controller = true;
...
bool AppSpineControl::setup()
{
    ...
    BigPuppySpineOnly* myModel = new BigPuppySpineOnly(segments, hips);
...
void AppSpineControl::handleOptions(int argc, char **argv)
{
...
const tgHillyGround::Config AppSpineControl::getHillyConfig()
{
...
const tgBoxGround::Config AppSpineControl::getBoxConfig()
{
...
tgModel* AppSpineControl::getBlocks()
{
...
tgWorld* AppSpineControl::createWorld()
{
...
tgSimViewGraphics *AppSpineControl::createGraphicsView(tgWorld *world)
{
    return new tgSimViewGraphics(*world, timestep_physics, timestep_graphics);
}

tgSimView *AppSpineControl::createView(tgWorld *world)
{
    return new tgSimView(*world, timestep_physics, timestep_graphics);
}

bool AppSpineControl::run()
{
    if (!bSetup)
    {
        setup();
...
void AppSpineControl::simulate(tgSimulation *simulation)
{
...
{
    std::cout << "AppSpineControl" << std::endl;
    AppSpineControl app (argc, argv);
