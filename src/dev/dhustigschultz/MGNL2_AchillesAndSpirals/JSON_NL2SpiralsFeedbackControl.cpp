...
/**
 * @file JSON_NL2SpiralsFeedbackControl.cpp
...
#include "JSON_NL2SpiralsFeedbackControl.h"
...
JSON_NL2SpiralsFeedbackControl::Config::Config(int ss,
                                        int tm,
                                        int om,
                                        int param,
                                        int segnum,
...
maxHeight(maxH),
minHeight(minH),
ourSpiralMuscles(osm),
theirSpiralMuscles(tsm),
/*ourLegMuscles(olm),
theirLegMuscles(tlm),
 ...
 */
JSON_NL2SpiralsFeedbackControl::JSON_NL2SpiralsFeedbackControl(JSON_NL2SpiralsFeedbackControl::Config config,	
                                                std::string args,
...
JSON_NL2SpiralsFeedbackControl::~JSON_NL2SpiralsFeedbackControl()
{
    //delete nn;
}

void JSON_NL2SpiralsFeedbackControl::onSetup(BaseQuadModelLearning& subject)
{
    ...
    Json::Value spiralNodeVals = root.get("spiralNodeVals", "UTF-8");
...
void JSON_NL2SpiralsFeedbackControl::onStep(BaseQuadModelLearning& subject, double dt)
{
    m_updateTime += dt;
    m_totalTime += dt;
...
void JSON_NL2SpiralsFeedbackControl::onTeardown(BaseQuadModelLearning& subject)
{
    scores.clear();
    metrics.clear();
    ...
    for(size_t i = 0; i < m_spiralControllers.size(); i++)
    {
        delete m_spiralControllers[i];
    }
    m_spiralControllers.clear();  
...
void JSON_NL2SpiralsFeedbackControl::setupCPGs(BaseQuadModelLearning& subject, array_2D achillesNodeActions, array_4D achillesEdgeActions, array_2D spiralNodeActions, array_4D spiralEdgeActions)
{
    ...
    std::vector <tgSpringCableActuator*> spiralMuscles = subject.find<tgSpringCableActuator> ("spiral");
    ...
    // Then spiral muscles from spine
    for (std::size_t i = 0; i < spiralMuscles.size(); i++)
    {

        tgPIDController::Config config(20000.0, 0.0, 5.0, true); // Non backdrivable
        tgCPGCableControl* pStringControl = new tgCPGCableControl(config);

        spiralMuscles[i]->attach(pStringControl);
        
        // First assign node numbers
        pStringControl->assignNodeNumberFB(m_CPGFBSys, spiralNodeActions);
        
        m_spiralControllers.push_back(pStringControl);
    }
	
    // Then determine connectivity and setup string
    for (std::size_t i = 0; i < m_spiralControllers.size(); i++)
    {
        tgCPGActuatorControl * const pStringInfo = m_spiralControllers[i];
        assert(pStringInfo != NULL);
        pStringInfo->setConnectivity(m_spiralControllers, spiralEdgeActions);
        
        //String will own this pointer
        tgImpedanceController* p_ipc = new tgImpedanceController( 0.0,
                                                        1000.0,
                                                        200.0); //Make these new config members eventually!
        if (m_config.useDefault)
        {
			pStringInfo->setupControl(*p_ipc);
		}
...
array_2D JSON_NL2SpiralsFeedbackControl::scaleNodeActions (Json::Value actions, double highFreq, double freqFeedbackMax)
{
...
array_4D JSON_NL2SpiralsFeedbackControl::scaleEdgeActions  
...
std::vector<double> JSON_NL2SpiralsFeedbackControl::getFeedback(BaseQuadModelLearning& subject)
{
    // Placeholder
...
std::vector<double> JSON_NL2SpiralsFeedbackControl::getCableState(const tgSpringCableActuator& cable)
{
...
std::vector<double> JSON_NL2SpiralsFeedbackControl::transformFeedbackActions(std::vector< std::vector<double> >& actions)
{
	// Placeholder
	std::vector<double> feedback;
