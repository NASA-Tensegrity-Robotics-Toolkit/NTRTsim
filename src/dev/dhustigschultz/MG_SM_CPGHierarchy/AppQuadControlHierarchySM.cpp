...
/**
 * @file AppQuadControlHierarchySM.cpp
...
#include "AppQuadControlHierarchySM.h"
#include "dev/btietz/JSONTests/tgCPGJSONLogger.h"

AppQuadControlHierarchySM::AppQuadControlHierarchySM(int argc, char** argv)
{
    bSetup = false;
    use_graphics = false;
    add_controller = true;
...
bool AppQuadControlHierarchySM::setup()
{
    ...
    MountainGoatSM* myModel = new MountainGoatSM(segments, hips, legs);
	...
	const double ffMax2 = 10.0;
        ...
        JSONHierarchyFeedbackControlSM::Config control_config(segmentSpan, 
                                                    numMuscles,
                                                    numMuscles,
                                                    numParams, 
                                                    segNumber, 
...
void AppQuadControlHierarchySM::handleOptions(int argc, char **argv)
{
...
const tgHillyGround::Config AppQuadControlHierarchySM::getHillyConfig()
{
...
const tgBoxGround::Config AppQuadControlHierarchySM::getBoxConfig()
{
...
tgModel* AppQuadControlHierarchySM::getBlocks()
{
...
tgWorld* AppQuadControlHierarchySM::createWorld()
{
...
tgSimViewGraphics *AppQuadControlHierarchySM::createGraphicsView(tgWorld *world)
{
    return new tgSimViewGraphics(*world, timestep_physics, timestep_graphics);
}

tgSimView *AppQuadControlHierarchySM::createView(tgWorld *world)
{
    return new tgSimView(*world, timestep_physics, timestep_graphics);
}

bool AppQuadControlHierarchySM::run()
{
    if (!bSetup)
    {
        setup();
...
void AppQuadControlHierarchySM::simulate(tgSimulation *simulation)
{
...
{
    std::cout << "AppQuadControlHierarchySM" << std::endl;
    AppQuadControlHierarchySM app (argc, argv);
