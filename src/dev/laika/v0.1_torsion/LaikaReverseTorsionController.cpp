...
/**
 * @file LaikaReverseTorsionController.cpp
 * @brief Implementation of LaikaReverseTorsionController.
...
LaikaReverseTorsionController::LaikaReverseTorsionController(double startTime,
						     double maxLength,
						     double rate,
  ...
  m_timePassed(0.0)
{
  ...
  // max length must be greater than 1.
    ...
    throw std::invalid_argument("maxLength is a percent, must be greater than 1. (100%)");
  }
 ...
 */
void LaikaReverseTorsionController::initializeActuators(TensegrityModel& subject,
						    std::string tag) {
  //DEBUGGING
 ...
 */
void LaikaReverseTorsionController::onSetup(TensegrityModel& subject)
{
...
void LaikaReverseTorsionController::onStep(TensegrityModel& subject, double dt)
{
  // First, increment the accumulator variable.
  m_timePassed += dt;
  // Then, if it's passed the time to start the controller,
  if( m_timePassed > m_startTime ) {
    // For each cable, check if its rest length is past the maximum,
    ...
    for (std::size_t i = 0; i < cablesWithTags.size(); i ++) {	
      ...
      // Remember that m_maxLength is a percent.
      double maxRestLength = initialRL[cablesWithTags[i]->getTags()] * m_maxLength;
      // If the current rest length is still less than the maximum,
      if( currRestLength < maxRestLength ) {
