...
#ifndef LAIKA_REVERSE_TORSION_CONTROLLER_H
#define LAIKA_REVERSE_TORSION_CONTROLLER_H

/**
 * @file LaikaReverseTorsionController.h
 * @brief Contains the definition of class LaikaReverseTorsionController.
 * @author Drew Sabelhaus, Ankita Joshi, Robel Teweldebirhan
 * $Id$
 */
...
/**
 * A controller for the horizontal spine (we're calling it 'Laika' now), that adjusts the 
 * lengths of its cables.
 */
class LaikaReverseTorsionController : public tgObserver<TensegrityModel>, public tgSubject<LaikaReverseTorsionController>
{
public:
	
  /**
   * Construct a LaikaReverseTorsionController.
   ...
   * @param[in] maxLength, a double that is the percent of the initial length
   * that this controller will increase to. Must be greater than 1.
   ...
   */
  LaikaReverseTorsionController(double startTime, double minLength, double rate,
			    std::vector<std::string> tagsToControl);
    
  /**
   * Nothing to delete, destructor must be virtual
   */
  virtual ~LaikaReverseTorsionController() { }
...
#endif // LAIKA_REVERSE_TORSION_CONTROLLER_H
