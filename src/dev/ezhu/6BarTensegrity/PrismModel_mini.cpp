...
void PrismModel::setup(tgWorld& world)
{
  ...
  */
  ...
  const tgRod::Config motorConfig(c.radius*2, c.density/2, c.friction, 
				c.rollFriction, c.restitution);
  const tgRod::Config tankConfig(tankRadius, 1355/pow(sf,3), c.friction, //density calculated so tank - 8.5 kg
  				 c.rollFriction, c.restitution);
  ...
  //				 c.rollFriction, c.restitution);
  /*
  const tgRod::Config linkConfig(c.radius/2.0, 0.001, c.friction, 
				 c.rollFriction, c.restitution);
