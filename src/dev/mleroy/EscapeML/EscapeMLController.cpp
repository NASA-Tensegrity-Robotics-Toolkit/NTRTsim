...
EscapeMLController::EscapeMLController(const double initialLength,
                                    std::string args,
...
void EscapeMLController::onSetup(EscapeMLModel& subject)
{
    m_totalTime = 0;
    double dt = 0.0001;
    ...
    for(int i=0; i<actions.size(); i++)
    {
        ...
        actions[i][2] = -1.79+i;
        actions[i][3] = 9;
        std::cout << actions[i][0] << " " << actions[i][1] << " " << actions[i][2] << " " << actions[i][3] << std::endl;
    }
...
void EscapeMLController::onStep(EscapeMLModel& subject, double dt)
{
    if (dt <= 0.0) {
...
void EscapeMLController::onTeardown(EscapeMLModel& subject) {
 ...
 */
std::vector< std::vector <double> > EscapeMLController::transformActions(std::vector< std::vector <double> > actions)
{
 ...
 */
void EscapeMLController::applyActions(EscapeMLModel& subject, std::vector< std::vector <double> > actions) {
...
void EscapeMLController::setupAdapter() {
    ...
    //std::cout << "Path: " << path << ", Name: " << configName << ", Config Anneal: " << configAnnealEvolution << std::endl;
...
double EscapeMLController::totalEnergySpent(EscapeMLModel& subject) {
...
void EscapeMLController::setPreferredMuscleLengths(EscapeMLModel& subject, double dt) {
...
void EscapeMLController::populateClusters(EscapeMLModel& subject) {
...
void EscapeMLController::initializeSineWaves() {
...
double EscapeMLController::displacement(EscapeMLModel& subject) {
...
std::vector<double> EscapeMLController::readManualParams(int lineNumber, std::string filename) {
    assert(lineNumber > 0);
...
void EscapeMLController::printCOM(EscapeMLModel& subject) {
...
void EscapeMLController::printMuscleTensions(EscapeMLModel& subject) {
    const std::vector<tgBasicActuator*> muscles = subject.getAllMuscles();
    for(size_t i=0; i<muscles.size(); i++) {
...
void EscapeMLController::printMuscleLengths(EscapeMLModel& subject) {
    const std::vector<tgBasicActuator*> muscles = subject.getAllMuscles();
    for(size_t i=0; i<muscles.size(); i++) {
...
void EscapeMLController::printSineParams() {
    for (size_t cluster = 0; cluster < clusters.size(); cluster++) {
