...
#ifndef PHASE_OSC_CONTROLLER_H
#define PHASE_OSC_CONTROLLER_H

/**
 * @file HorizontalSpineController.h
 * @brief Contains the definition of class PhaseOscController
 * @author Marc Leroy
 * $Id$
 */
 ...
 */
class PhaseOscController : public tgObserver<TensegrityModel>, public tgSubject<PhaseOscController>
{
public:
    ...
    struct Config
    {
      public:
           ...
           */
          Config(double hOMin, double hOMax, double hMMin, double hMMax, double cUpMin, double cUpMax, 
      ...
      const double couplingUpMin;
      const double couplingUpMax;
      const double couplingDownMin;
      const double couplingDownMax;
      const double couplingNeMin;
      const double couplingNeMax;
      const double couplingSeMin;
      const double couplingSeMax;
  ...
  //virtual void odeTest(const state_type &xPO, state_type &dxdt, const double t);
  
  
protected:
  PhaseOscController::Config m_config;
...
private:
  ...
  double couplingDown[NOSCILLATORS];
  double couplingNe[NOSCILLATORS];
  double couplingSe[NOSCILLATORS];
  double hopfOffsetEven[NOSCILLATORS];
  double hopfOffsetOdd[NOSCILLATORS];

  double couplingArray[NOSCILLATORS-2];
  int coupledState[NOSCILLATORS-1];
