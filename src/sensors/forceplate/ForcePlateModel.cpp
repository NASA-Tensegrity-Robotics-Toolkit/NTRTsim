...
void ForcePlateModel::setup(tgWorld& world)
{
  ...
  // Config for the housing walls:
  // @TO-DO: CHANGE DENSITY. Right now, since this class only supports
  //    a mass of 0 for the outer housing, the density doesn't matter.
  //    But, with a possibly-moving force plate in the future, this density
  //    will have to be corrected by the factor of 1/2 for the half-extents too.
...
void ForcePlateModel::onVisit(tgModelVisitor& v)
{
  // Need to call the model visitor's render function in order for
  // this object to be rendered. This is how the tgDataLogger
  // accesses the forces inside this force plate.
  //DEBUGGING
  std::cout << "Called onVisit inside ForcePlateModel" << std::endl;
  v.render(*this);
  // tgModel's onVisit function calls the onVisit of its children.
  // We don't want to output the raw data from the springs,
  // so don't call it.
  //tgModel::onVisit(v);
}
...
double ForcePlateModel::getPlateGravitationalForce() const
{
  ...
  // @TODO: CALIBRATION! This is off by some factor, and Drew is not sure
  // if that's an artifact of the simulation, or because the math is somehow
  // incorrect somewhere. TEST THIS.
...
double ForcePlateModel::getFy() const
{
...
double ForcePlateModel::getFz() const
{
